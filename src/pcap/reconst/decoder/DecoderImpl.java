...
public class DecoderImpl implements Decoder {
    ...
    public DecodedData decode(byte[] streamToBeDecoded, Headers headers, TimestampPair ts) {
    	if(log.isDebugEnabled()){
    		log.debug("length is [" + streamToBeDecoded.length + 
    				"], data is [" + new String(streamToBeDecoded) + "]");
    	}
        ...
        if (headers != null) {
            ...
            if (Utils.isCompressed(compressionType)) {
                ...
                if(log.isDebugEnabled()){
                	log.debug("after gunzip: length is [" + streamToBeDecoded.length + "], data is [" + 
