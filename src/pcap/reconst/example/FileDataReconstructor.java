...
public class FileDataReconstructor {
    ...
    public static void main(String[] args) {
        try {
            ...
            Map<TcpConnection, TcpReassembler> map = fileDataReconstructor.reconstruct(new File(args[0]), new JpcapReconstructor(new PacketReassembler()));
            ...
            for (TcpConnection tcpConnection : httpPackets.keySet()) {
                ...
                List<HttpOutput> httpOutputList = httpPackets.get(tcpConnection);
            ...
            for (TcpConnection tcpConnection : decodedPackets.keySet()) {
                System.out.println(tcpConnection);
                List<HttpDecodedOutput> decodedList = decodedPackets.get(tcpConnection);
